...
# Uses BuildKit-compatible flags for parallel builds and better caching.
...
if [ "$BUILDKIT" = "true" ]; then
    BUILDKIT_FLAGS="--format docker --layers --force-rm"
fi
...
if [ $? -eq 0 ]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        if [ -z "$DOCKERHUB_USER" ]; then
            ...
            # Tag for Docker Hub
            echo "Tagging image..."
            podman tag "$IMAGE_NAME:$IMAGE_TAG" "docker.io/$DOCKERHUB_USER/$IMAGE_NAME:$IMAGE_TAG"
            
            # Push to Docker Hub
            echo "Pushing to Docker Hub..."
            if podman push "docker.io/$DOCKERHUB_USER/$IMAGE_NAME:$IMAGE_TAG"; then
                echo ""
                echo "✓ Successfully pushed to: docker.io/$DOCKERHUB_USER/$IMAGE_NAME:$IMAGE_TAG"
            else
                echo "✗ Failed to push image. Make sure you are logged in: podman login docker.io"
            fi
        fi
    fi
